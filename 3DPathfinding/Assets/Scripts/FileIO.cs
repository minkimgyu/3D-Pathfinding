...
public class FileIO
{
    ...
    public SingleThread.Node[,,] LoadData(string filePath, string fileName)
    {
        string path = ReturnPath(filePath, fileName);
        if (File.Exists(path) == false) return default;

        string jData = File.ReadAllText(path);
        return _parser.JsonToObject(jData);
    }

    public void SaveData(SingleThread.Node[,,] objectToParse, string filePath, string fileName, bool canOverwrite = false)
    {
        string path;
